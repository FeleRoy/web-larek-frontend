...
Данный класс отвечает за отображение данных страницы: список карточек, корзину, кол-во товаров в корзине. Управляет отображением карточек товаров на главной странице, а также кнопкой корзины и счетчиком товаров в корзине. 
...
Отвечает за отображение товара, задавая в карточке данные. Класс используется для отображения товаров на странице сайта. 
...
Класс отвечает за отображение и управление модальными окнами. Методы `open` и `close` контролируют отображение окна, а также установлены слушатели для закрытия окна при клике на оверлей или кнопку-крестик.\
...
#### Класс Form
Класс отвечает за управлением логикой форм, отображаемых в модальном окне. Он обрабатывает поля формы, проверяет их заполненность, управляет состоянием кнопки отправки и генерирует события при отправке данных./
Поля класса содержат элемент формы и кнопку отправки. Конструктор, кроме селектора для идентификации формы на странице принимает экземпляр `EventEmitter` для инициации событий.\
Методы:
- isValid(): boolean — Проверяет, заполнены ли все обязательные поля формы. Используется для активации кнопки 
- clear(): void - Очищает содержимое формы и деактивирует кнопку отправки

#### Класс Basket

Класс отвечает за управляет отображением корзины товаров в модальном окне и взаимодействием с её содержимым. Корзина отображает список добавленных товаров, их стоимость, общую cумму

Поля класса содержат контейнер для списка товаров в корзине, элемент для отображения общей стоимости товаров и кнопку оформления. Конструктор, кроме селектора для идентификации корзины на странице принимает экземпляр `EventEmitter` для инициации событий.\
Методы:
...
Класс управляет модальным окном, которое отображает сообщение об успешном завершении заказа. 

Поля класса содержат элемент для отображения итоговой суммы заказа и кнопку для закрытия модального окна. Конструктор, кроме селектора для идентификации окна на странице принимает экземпляр `EventEmitter` для инициации событий.
